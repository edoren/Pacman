...
void IntroState::init(ResourceManager* resources) {
    sfml_texture_ = new sf::Texture();
    sfml_texture_->loadFromFile("assets/images/sfml-logo.png");

    background_ = new sf::RectangleShape(sf::Vector2f(224, 288));
    background_->setFillColor(sf::Color::Black);

    sfml_logo_ = new sf::Sprite(*sfml_texture_);
    sfml_logo_->setOrigin(sf::Vector2f(sfml_texture_->getSize().x, sfml_texture_->getSize().y) / 2.f);
